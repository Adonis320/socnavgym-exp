...
class MSR:
    ...
    def __init__(
        ...
        self.R_topo = defaultdict(float)
        self.R_social = defaultdict(float)
    ...
    def _q_values(self, state_key):
        ...
        where:
        Q_topo(s,a)   = sum_{t'} SR_topo[topo][a][t']   * R_topo[t']
        Q_social(s,a) = sum_{u'} SR_social[soc][a][u'] * R_social[u']
        """
        ...
        for a in range(self.action_size):
            ...
            for t_prime, m_t in self.SR_topo[topo_key][a].items():
                q += m_t * self.R_topo[t_prime]

            # social contribution
            for s_prime, m_s in self.SR_social[social_key][a].items():
                q += m_s * self.R_social[s_prime]
    ...
    def update_reward(self, state_key, reward, upd_social=True):
        """
        Update R_topo and R_social from the observed immediate reward.

        Reward is assumed to be the same scalar for both branches, as in your
        original MSR code. No action dependence.
        """
        topo_key, social_key = state_key

        # topographic reward
        rt = self.R_topo[topo_key]
        self.R_topo[topo_key] = rt + self.r_learning_rate_topo * (reward - rt)

        # social reward
        if upd_social:
            rs = self.R_social[social_key]
            self.R_social[social_key] = rs + self.r_learning_rate_social * (reward - rs)

    # --------------
    # Interaction loop
    # --------------

    def act(self, env, obs, eval=False, upd_social=False):
        """
        ...
        while True:
            ...
            self.update_reward(next_state_key, reward, upd_social=upd_social)
