...
if args["api_key"] is not None:
    if comet_ml is None:
        raise ImportError("comet_ml not installed. Please install with: pip install comet_ml")
...
config = DSRConfig(
    ...
    feat_dim=64,
    gamma=0.99,
    lambda_r=2.0,          # was 0.5 — encoder must be reward-predictive
    lambda_sr=0.5,
    lambda_recon=0.1,      # was 0.5 — recon is auxiliary, don't let it dominate
    lambda_var=0.1,        # new — phi collapse prevention
    lr_rw=5e-4,
    lr_sr=1e-4,
    grad_clip=0.5,
...
ep_ret = 0.0
ep_len = 0
last_loss_sr: Optional[float] = 0.0
last_loss_r: Optional[float] = 0.0
last_loss_recon: Optional[float] = 0.0
...
while total_steps < args["total_steps"]:
    ...
    if total_steps % args["log_freq"] == 0:
        metrics = {
            "rollout/ep_rew_mean": float(np.mean(ret_window)) if len(ret_window) > 0 else 0.0,
            "rollout/ep_len_mean": float(np.mean(len_window)) if len(len_window) > 0 else 0.0,
            "rollout/epsilon": agent.epsilon,
            "rollout/buffer_size": len(agent.rb),
            "train/loss_sr": last_loss_sr if last_loss_sr is not None else 0.0,
            "train/loss_r": last_loss_r if last_loss_r is not None else 0.0,
            "train/loss_recon": last_loss_recon if last_loss_recon is not None else 0.0,
        }
